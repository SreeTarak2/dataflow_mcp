...
ALLOWED_PATCH_FIELDS: Set[str] = {
    # Prize fields
    'prize.isMonetary', 'prize.originalAmount', 'prize.totalUSD',
    'prize.currency', 'prize.prizeSummary', 'prize.description',
    'prize.breakdown',
    # Entry fields
    'entry.isFree', 'entry.feeConfidence', 'entry.feeNote',
    'entry.fee.amount', 'entry.fee.currency',
    # Description fields
    'description', 'descriptionDetailed',
    # Audience fields
    'audience.eligibilityLabel', 'audience.eligibilityDetail',
    'audience.primarySkillLevel', 'audience.skillLevelSource',
    'audience.location', 'audience.mode',
    'audience.age.min', 'audience.age.max',
    # Category fields
    'category', 'subCategory', 'rawCategory',
    # Tags
    'tags',
    # Timeline
    'timeline.submissionDeadlineUTC', 'timeline.startDateUTC',
    'timeline.eventEndUTC',
    # Image
    'image.primary.url', 'image.alt',
    # Type
    'type',
    # Link
    'link',
...
VALID_FEE_CONFIDENCE: Set[str] = {
    'confirmed', 'extracted', 'unknown',
}

VALID_MODES: Set[str] = {
    'online', 'offline', 'in-person', 'hybrid',
}
...
CATEGORY_TO_SUBCATEGORIES: Dict[str, Set[str]] = {
    'Creative Arts': {
        'Photography', 'Illustration & Visual Art', 'Graphic Design',
        'Fashion Design', 'Architecture & Urban Design',
    },
    'Technology & AI': {
    ...
    'Writing & Media': {
        'Fiction & Creative Writing', 'Film & Video',
        'Music & Audio', 'Journalism & Nonfiction',
    },
    'Environment & Sustainability': {
        'Conservation & Ecology', 'Climate & Clean Energy',
    ...
    'Social Impact & Leadership': {
        'Community & Equity', 'Youth Leadership',
    },
...
class PatchValidator:
    ...
    @staticmethod
    def _check_string(value: Any, field_name: str,
                      min_len: int = 0, max_len: int = 1000) -> Optional[str]:
    ...
    def check_schema_compliance(patch: Dict[str, Any]) -> PatchValidationResult:
        ...
        schema_checks = {
            ...
            'prize.originalAmount': lambda v: (
                None if v is None or isinstance(v, (int, float, str))
            ...
            'prize.currency': lambda v: PatchValidator._check_string(v, 'prize.currency', max_len=5),
            'prize.prizeSummary': lambda v: PatchValidator._check_string(v, 'prize.prizeSummary', max_len=300),
            'prize.description': lambda v: PatchValidator._check_string(v, 'prize.description', max_len=1000),
            'prize.breakdown': lambda v: PatchValidator._check_string(v, 'prize.breakdown', max_len=500),
            # Entry
            'entry.isFree': lambda v: PatchValidator._check_bool(v, 'entry.isFree'),
            'entry.feeConfidence': lambda v: PatchValidator._check_enum(v, VALID_FEE_CONFIDENCE, 'entry.feeConfidence'),
            'entry.feeNote': lambda v: PatchValidator._check_string(v, 'entry.feeNote', max_len=500),
            'entry.fee.amount': lambda v: PatchValidator._check_number(v, 'entry.fee.amount'),
            'entry.fee.currency': lambda v: PatchValidator._check_string(v, 'entry.fee.currency', max_len=5),
            # Description
            'description': lambda v: PatchValidator._check_string(v, 'description', max_len=500),
            'descriptionDetailed': lambda v: PatchValidator._check_string(v, 'descriptionDetailed', max_len=2000),
            # Audience
            'audience.eligibilityLabel': lambda v: PatchValidator._check_string(v, 'audience.eligibilityLabel', max_len=150),
            'audience.eligibilityDetail': lambda v: PatchValidator._check_string(v, 'audience.eligibilityDetail', max_len=500),
            'audience.primarySkillLevel': lambda v: PatchValidator._check_enum(v, VALID_SKILL_LEVELS, 'audience.primarySkillLevel'),
            'audience.skillLevelSource': lambda v: PatchValidator._check_enum(v, VALID_SKILL_LEVEL_SOURCES, 'audience.skillLevelSource'),
            'audience.location': lambda v: PatchValidator._check_string(v, 'audience.location', max_len=100),
            ...
            # Timeline
            'timeline.submissionDeadlineUTC': lambda v: PatchValidator._check_string(v, 'timeline.submissionDeadlineUTC', max_len=30),
            'timeline.startDateUTC': lambda v: PatchValidator._check_string(v, 'timeline.startDateUTC', max_len=30),
            'timeline.eventEndUTC': lambda v: PatchValidator._check_string(v, 'timeline.eventEndUTC', max_len=30),
            # Image
            'image.primary.url': lambda v: PatchValidator._check_string(v, 'image.primary.url', max_len=2000),
    ...
    def check_destructive_write(
        ...
        if existing_doc is None:
            ...
            result.warnings.append(
                "Destructive write check skipped: no existing document provided"
            )
            return result
    ...
    def check_cross_field_consistency(patch: Dict[str, Any]) -> PatchValidationResult:
        ...
        if is_monetary is False and prize_summary:
            cash_keywords = ['cash', 'prize', '€', '$', '£', '₹', '₦', 'dollar', 'euro', 'won', 'grant']
...
class ContestMigration:
    ...
    def get_migration_status() -> Dict[str, Any]:
        ...
        try:
            ...
            missing_sub_category = collection.count_documents({
                "subCategory": {"$exists": False}
            })
    ...
    def apply_migration_patch(
        ...
        try:
            ...
            existing_doc = collection.find_one(
                {"_id": doc_id},
                {
                    "prize": 1, "entry": 1, "audience": 1,
                    "description": 1, "descriptionDetailed": 1,
                    "category": 1, "subCategory": 1,
                    "tags": 1, "type": 1, "link": 1,
                    "timeline": 1, "image": 1,
                }
            )
    ...
    def bulk_apply_migrations(
        ...
        try:
            ...
            for migration in migrations:
                ...
                result = ContestMigration.apply_migration_patch(
                    contest_id, patch, force=item_force
                )
